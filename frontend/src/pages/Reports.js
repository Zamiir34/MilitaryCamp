...
export default function Reports() {
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="print-only" style={{ marginBottom: '2rem', borderBottom: '3px double #000', paddingBottom: '1rem', textAlign: 'center' }}>
        <h1 style={{ fontSize: 24, fontWeight: 'bold', textTransform: 'uppercase', margin: 0 }}>Camp Security Access Control System</h1>
        <h2 style={{ fontSize: 16, textTransform: 'uppercase', margin: '5px 0 10px' }}>Access Movement Security Audit Report</h2>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginTop: 15, padding: '0 1rem' }}>
      ...
      {report && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
            {[
              { label: 'Total Logs', value: s?.total, color: 'var(--text-primary)' },
              ...
              { label: 'Violations', value: s?.unauthorized, color: 'var(--accent-red)', alert: s?.unauthorized > 0 },
            ].map(item => (
              <div 
                key={item.label} 
                className="stat-card" 
                style={{ 
                  ...
                  padding: '1rem',
                  boxShadow: item.alert ? 'var(--glow-red)' : 'none',
                  borderColor: item.alert ? 'var(--accent-red)' : undefined
                }}
              >
                <div className="stat-label">{item.label}</div>
                <div className="stat-value" style={{ color: item.color, fontSize: 28, fontWeight: 900 }}>
                  {item.value}
                </div>
              </div>
            ))}
